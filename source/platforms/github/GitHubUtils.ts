...
export const createUpdatedIssueWithIDGenerator =
  (api: GitHub) =>
  async (id: string, content: string, settings: { title: string; open: boolean; owner: string; repo: string }) => {
    // Could also scope:
    //   by author
    //   by label
...
export const createOrUpdatePR =
  (pr: GitHubPRDSL | undefined, api: GitHub) => async (config: PRCreationConfig, fileMap: any) => {
